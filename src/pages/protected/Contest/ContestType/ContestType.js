...
const ContestType = (props) => {
  ...
  console.log("🚀 ~ file: ContestType.js ~ line 14 ~ ContestType ~ item", item?.title)
  ...
  const handleApi = async () => {
    try{
      setLoading(true)
      const data = await ApiSauce.getWithoutToken(GET_CONTEST(item.title == 'Monthly Contest' ? 'monthly' : item.title == 'Yearly Contest' ? 'yearly' : 'video'))
      setData(data.data)
  ...
  const renderItem = ({ item, index }) => {
    console.log("🚀 ~ file: ContestType.js ~ line 106 ~ renderItem ~ item", item)
    return (
        <CListItem
